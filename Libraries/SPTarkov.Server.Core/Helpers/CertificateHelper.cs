...
namespace SPTarkov.Server.Core.Helpers
{
  ...
  {
    const string certificatePath = "./user/certs/server.crt";
    const string certificateKeyPath = "./user/certs/server.key";
    const string certificatePfxPath = "./user/certs/certificate.pfx";
    ...
    /// </summary>
    /// <returns></returns>
    bool TryLoadCertificatePfx(out X509Certificate2? certificate)
    {
    ...
    X509Certificate2? LoadCertificatePfx()
    {
      try
      {
        //Archangel: For some reason despite this being deprecated this is the only way to load a certificate file
    ...
    /// </summary>
    /// <param name="certificate">Certificate to save</param>
    void SaveCertificatePfx(X509Certificate2 certificate)
    {
      try
      {
    ...
    void SavePrivateKey(RSA privateKey)
    {
      try
      {
        var privateKeyBytes = privateKey.ExportPkcs8PrivateKey();

        // Convert the key to PEM format (Base64 encoded)
