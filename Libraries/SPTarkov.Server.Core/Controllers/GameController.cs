...
)
{
  ...
  protected void LogProfileDetails(SptProfile fullProfile)
  {
    if (_logger.IsLogEnabled(LogLevel.Debug))
    {
      ...
      _logger.Debug($"Server version: {_settings.SptVersion ?? _coreConfig.SptVersion} {_settings.Commit}");
#if DEBUG
      _logger.Debug($"Debug enabled: true");
#else
      _logger.Debug($"Debug enabled: false");
#endif
      _logger.Debug($"Mods enabled: {_settings.ModsEnabled}");
    }
  }
