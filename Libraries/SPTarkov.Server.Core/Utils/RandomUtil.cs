...
{
  const int DecimalPointRandomPrecision = 6;
  ...
  static readonly int DecimalPointRandomPrecisionMultiplier = (int) Math.Pow(10, DecimalPointRandomPrecision);
  ...
  double GetBoundedGaussian(double start, double end, double n)
  {
    return Math.Round(start + GetGaussianRandom(n) * (end - start + 1));
  }

  double GetGaussianRandom(double n)
  {
    var rand = 0d;
    for (var i = 0; i < n; i += 1)
    {
