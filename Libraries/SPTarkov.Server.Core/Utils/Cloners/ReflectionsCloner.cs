...
{
  static Dictionary<Type, MemberInfo[]> MemberInfoCache = new();
  static Dictionary<Type, MethodInfo> AddMethodInfoCache = new();
  ...
  static ConcurrentDictionary<Type, PropertyInfo> _itemPropertyInfoCache = new();
  static ConcurrentDictionary<Type, PropertyInfo> _listPropertyInfoCache = new();

  async Task<object?> HandleSpecialClones(object obj, Type objectType)
  {
    if (objectType.IsGenericType && objectType.GetGenericTypeDefinition() == typeof(ListOrT<>))
    {
