...
{
  const string _systemSenderId = "59e7125688a45068a6249071";
  ...
  Message CreateDialogMessage(string dialogId, SendMessageDetails messageDetails)
  {
    Message message = new()
    {
      Id = _hashUtil.Generate(),
  ...
  void AddRewardItemsToMessage(Message message, MessageItems? itemsToSendToPlayer, long? maxStorageTimeSeconds)
  {
    if ((itemsToSendToPlayer?.Data?.Count ?? 0) > 0)
    {
  ...
  MessageItems ProcessItemsBeforeAddingToMail(MessageType? dialogType, SendMessageDetails messageDetails)
  {
  ...
  Item GetBaseItemFromRewards(List<Item>? items)
  {
  ...
  Dialogue GetDialog(SendMessageDetails messageDetails)
  {
  ...
  string? GetMessageSenderIdByType(SendMessageDetails messageDetails)
  {
